...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while nearest_bee_place is not hive:
            possible_bee = random_or_none(nearest_bee_place.bees)
            if possible_bee:
                return possible_bee
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_true_queen:
            ...
            while place_behind:
                if place_behind.ant:
                    if place_behind.ant.is_container and place_behind.ant.contained_ant\
                                       and not place_behind.ant.contained_ant.is_buffed:
                        place_behind.ant.contained_ant.damage *= 2
                        place_behind.ant.contained_ant.is_buffed = True
                    if not place_behind.ant.is_buffed:
                        place_behind.ant.damage *= 2
                        place_behind.ant.is_buffed = True
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    new_action, old_action = effect(bee.action, bee), bee.action
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, hive):
        # BEGIN Problem OPTIONAL
        insect_dict = {}
        starting_place, distance_from_start = self.place, 0
        while starting_place is not hive:
            for bee in starting_place.bees:
                insect_dict[bee] = distance_from_start
            if starting_place.ant and starting_place.ant is not self:
                insect_dict[starting_place.ant] = distance_from_start
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        updated_damage = self.damage - self.insects_shot * 0.05 - distance * 0.2
        if updated_damage < 0:
            updated_damage = 0
